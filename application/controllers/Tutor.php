...
$tutor[0]->class_schedule  = json_encode($userData);
      ...
      echo json_encode(array('class_schedule' => $tutor[0]->class_schedule, 'tutor' => $tutor[0], 'date' => $currentDate ));
    ...
    if($this->jwt->getToken()){
        ...
        foreach($Tutors as $tutor){
          ...
          foreach($schedules as $schedule){
            if($schedule->schedule->category === 'regularclass' && $schedule->active === true){
   ...
   break;
    ...
    break;
        ...
        foreach($tutors as  $k => $tutor){
          ...
          if(array_key_exists('overlap', $tutor)){
  ...
  public function changeTeacher(){
   ...
   break;
            ...
            foreach($tutors as  $k => $tutor){
              ...
              if(array_key_exists('overlap', $tutor) || $tutors[$k]->idx === $currentTutor){
...
if($currentTutorSchedules[$k]->cancelled_class){
...
$evaluation = '';
    ...
    $tutor =   $this->tutor->getTutor($user[0]->idx)[0];
      ...
      foreach($schedules as $k => $value){
        ...
        $evaluation = $this->evaluation->getEvaluation($classId)[0];
        if($evaluation){
...
if($schedules){
      foreach($schedules as $k => $value){
        ...
        $evaluation->evaluation = json_decode($evaluation->evaluation);
          ...
          $evaluation = $this->evaluation->getleveltestEvaluation($class->schedule->preference_number)[0];
...
if($schedules){
  ...
  public function SaveCurriculim(){
